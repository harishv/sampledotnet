...
function mouseOverImage(id) {
	...
	var doc_id = str.substr(0, (str.length - 1));

	var n = str.substr(-1, 1);

	for (i = 1; i <= n; i++) {
		$('img#full_off' + doc_id + i).attr('src', base_url + 'img/star-full.png');
		$('img#full_on' + doc_id + i).attr('src', base_url + 'img/star-full.png');
	}

	for (var j = 5; j > n; j--) {
		$('img#full_off' + doc_id + j).attr('src' ,base_url + 'img/star-off.png');
		$('img#full_on' + doc_id + j).attr('src' ,base_url + 'img/star-off.png');
	}
}
...
function mouseOutImage(id) {
	...
	for (var i = 1; i <= 5; i++) {
		$('img#full_off' + doc_id + i).attr('src' ,base_url + 'img/star-off.png');
		$('img#full_on' + doc_id + i).attr('src' ,base_url + 'img/star-full.png');
	}
}

function doc_rating(id, count) {
	var data = {
		'doc_id': id,
		'vote_value': count
	};
	...
	$.ajax({
		url: base_url + 'documents/document_rating',
		type: 'POST',
		data: data,
		dataType: 'json',
